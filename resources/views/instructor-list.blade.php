...
<div class="mb-6">
    <label class="flex items-center space-x-2 cursor-pointer">
        <input type="checkbox" id="filter-city" class="h-5 w-5 text-yeng-pink-500 focus:ring-yeng-pink-500 rounded border-gray-500">
        <span class="font-bold text-sm text-gray-500">From My City</span>
    </label>
</div>
    ...
    @foreach ($instructors as $instructor)
        <div class="instructor-item bg-white p-6 py-4 mt-4 rounded-lg shadow-lg border-yeng-pink-500 border-2 cursor-pointer" data-city="{{ $instructor->city }}">
            ...
            <div class="flex text-sm font-bold mt-4 justify-end">
                <!-- View Details Button -->
                <button onclick="window.location.href='{{ route('instructors.show', ['id' => $instructor->id]) }}'" class="font-raleway text-yeng-pink-500 border-yeng-pink-500 bg-white border-2 p-6 py-2 ml-2 rounded-full">
                    View
                </button>

                <!-- Book Button -->
                <button onclick="event.stopPropagation(); window.location.href='{{ route('booking', ['instructor_name' => $instructor->name, 'instructor_id' => $instructor->id]) }}'" class="font-raleway text-white border-yeng-pink-500 bg-yeng-pink-500 border-2 p-6 py-2 ml-2 rounded-full">
                    Book
                </button>
            </div>
        </div>
